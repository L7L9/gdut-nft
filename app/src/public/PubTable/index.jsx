...
class PubTable extends Component {
    ...
    handledata = (data) => {
        ...
        markID!=='activity'?data.map((item, index) => {
            datasource.push({
                preview: item.url,
                name: item.nftName,
                number: markID!=='nftsearch'&&markID!='mynft'?item.cid:item.tokenId,
    ...
    returnData = () => {
        ...
        markID === 'mynft' ? refresh.message.mynft :
        markID === 'mysell' ? refresh.message.mysell :
    ...
    componentDidUpdate(preprops) {
        ...
        const changetable=JSON.parse(sessionStorage.getItem('changetable'))
        if (changetable) {
            const { alldata, markID, updatedata } = this.props
            let data1 = alldata[markID]
            const tabledata = this.handledata(data1)
            updatedata({ ...alldata, currentdata: data1 });
            this.setState({ data: tabledata })
            sessionStorage.setItem('changetable',JSON.stringify(false))
        }
