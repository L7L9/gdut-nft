...
class Content extends Component {
    ...
    showmynft = () => {
        ...
        changeloading(true)
        const myrefresh=refresh.message
        pageModel.showMyNFT().then(res => {
            updatedata({ ...alldata, [markID]: res,currentdata:res })
            this.setState({ data: res })
            changeloading(false)
            changerefresh({...refresh,message:{...myrefresh,mynft:false}})
        })
    }
    showmysell = () => {
        const { markID, alldata, updatedata, changerefresh, refresh, changeloading } = this.props
        ...
        pageModel.showMySell().then(res => {
            ...
            this.setState({ data: res })
            changeloading(false)
            changerefresh({...refresh,message:{...myrefresh,mysell:false}})
        })
    }
    showactivity = () => {
        const { markID, alldata, updatedata,changerefresh,refresh,changeloading } = this.props
    ...
    componentDidMount() {
        ...
        :
        markID === 'mynft' ?
            (this.props.refresh.message.mynft ? this.showmynft() :
            (
            timer = setInterval(() => {
                const { contentloading } = this.props
                if (!contentloading) {
        ...
        :
        markID === 'mysell' ?
            (this.props.refresh.message.mysell ? this.showmysell() :
            (
            timer = setInterval(() => {
                const { contentloading } = this.props
                if (!contentloading) {
